...
import { ModelCatalog, ModelCatalogLive, ModelCatalogTest, type ModelCatalogShape } from "./model-catalog.ts";
 ...
 */
export const makeSakuDaemon = Effect.fn("makeSakuDaemon")(function* (options: {
  ...
  const resolveThreadId = Effect.fn("resolveThreadId")(function* (input: string) {
    ...
    if (Result.isFailure(resolved)) {
      return yield* Effect.fail(new DaemonError({ code: "resolution", message: resolved.failure }));
    }
  ...
  const handleSessionCommand = Effect.fn("handleSessionCommand")(function* (
    ...
    return yield* runSessionCommand({ hostFor, readOnlyHost, availableModels }, threadId, command);
  });
