...
import { DeletedThread, GotPickerDialogMessage } from "./message.ts";
import type { RailMessage, RailOutMessage } from "./message.ts";
import type { Model } from "./model.ts";
import { browseEntries, initialPicker, projectSessions, projects, threadList } from "./model.ts";

export type Commands = readonly Command.Command<RailMessage, never, Wire>[];
export type UpdateReturn = readonly [Model, Commands, Option.Option<RailOutMessage>];

const none: Commands = [];

/** Drop one key from a record (the destructuring-rest shape, avoiding a
 *  dynamic delete on the copied map). */
const omitKey = <T>(map: Readonly<Record<string, T>>, key: string) =>
...
const upsertProject = (model: Model, project: ProjectInfo) => {
  if (model.projects._tag !== "Success") {
    return model;
  }
...
export const update = (model: Model, message: RailMessage) =>
  M.value(message).pipe(
    ...
    M.tagsExhaustive({
      ...
      GotPickerDialogMessage: ({ message: dialogMessage }) => {
        const [dialog, dialogCommands, maybeOut] = Dialog.update(model.dialog, dialogMessage);
      ...
      PickerAddRequested: ({ path }) => {
        ...
        if (model.picker.entries._tag !== "Success") {
          return [model, none, Option.none()];
        }
      ...
      PickerHighlightMoved: ({ delta }) => {
        ...
        if (next === model.picker.highlight) {
          return [model, none, Option.none()];
        }
        return [
      ...
      ProjectRemoved: ({ path }) => {
        ...
        return [
          evo(model, {
            expanded: (_) => omitKey(model.expanded, path),
            projectSessions: (_) => omitKey(model.projectSessions, path),
            projects: (_) => projects_,
            sessionShowMore: (_) => omitKey(model.sessionShowMore, path),
          }),
          none,
          Option.none(),
        ];
      ...
      ThreadRenameFailed: ({ error }) => [
        evo(model, { notice: (_) => error.message, renaming: (_) => null }),
        none,
        Option.none(),
      ],
